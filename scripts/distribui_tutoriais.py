...
def trata_entrada():
	...
	entrada = entrada[entrada[entrada.columns[22]]=="Tutorial"].copy()
	entrada[entrada.columns[3]]=entrada[entrada.columns[3]].apply(lambda x : x.strip())
	entrada = entrada.drop(columns=entrada.columns.values[4:32])
	entrada = entrada.drop(columns=entrada.columns.values[12:])
